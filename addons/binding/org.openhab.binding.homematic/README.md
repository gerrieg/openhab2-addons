...
All gateways which provides the Homematic BIN-RPC API: 
- CCU 1+2 
...
```
homematic:TYPE:BRIDGE:SERIAL:[V|M]_CHANNELNUMBER_DATAPOINTNAME
```
* **homematic:** the binding id, fixed  
* **type:** the type of the Homematic device  
* **bridge:** the name of the bridge  
* **serial:** the serial number of the Homematic device  
* **V|M:** V = Values, M = Master parameterset. With the Master parameterset you can change all device settings
* **channelnumber:** the channel number of the Homematic datapoint
* **datapointname:** the name of the Homematic datapoint

```
Switch  RC_1  "Remote Control Button 1" { channel="homematic:HM-RC-19-B:ccu:KEQ0099999:V_1_PRESS_SHORT" }
Dimmer  Light "Light [%d %%]"           { channel="homematic:HM-LC-Dim1T-Pl-2:ccu:JEQ0555555:V_1_LEVEL" }

# change device settings
Number  Event_Delay "Delay [%d %%]"     {channel="homematic:HM-Sec-RHS:ccu:KEQ00333333:M_1_EVENT_DELAYTIME" }
```
...
#### RELOAD_FROM_GATEWAY
A virtual datapoint (Switch) to reload all values for the device, available in channel 0 for each device

#### FIRMWARE
A virtual datapoint (String) to show the firmware version of the device, available in channel 0 for each device

#### BATTERY_TYPE
A virtual datapoint (String) to show the battery type, available in channel 0 for each battery powered device
...
```
String Display_Options "Display_Options" { channel="homematic:HM-RC-19-B:ccu:KEQ0099999:V_18_DISPLAY_OPTIONS" }
```
show message TEST:
```
